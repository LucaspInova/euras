...
const PARTNER_CREATE_FUNCTION_NAME = "criar-parceiro";
const PARTNER_CREATE_TIMEOUT_MS = 22000;
...
function createPartnerFunctionTimeout() {
  const error = new Error(
    "Tempo limite ao criar o parceiro com seguranca no servidor.",
  );
  error.code = "partner_create_timeout";
  return error;
}
...
async function parseFunctionHttpError(error) {
  ...
  const message =
    ...
    payload?.message ??
    (status === 404
      ? "Funcao segura de cadastro de parceiro nao encontrada."
      : "Falha ao criar parceiro pelo endpoint seguro.");
...
async function normalizePartnerCreateFunctionError(error) {
  ...
  if (error instanceof FunctionsFetchError) {
    const wrappedError = new Error(
      "Nao foi possivel conectar ao endpoint seguro de cadastro de parceiro.",
    );
  ...
  return error instanceof Error
    ? error
    : new Error("Falha inesperada ao criar parceiro no servidor.");
}

async function invokeSecurePartnerCreate(payload, session) {
  let timeoutId = null;
  const timeoutPromise = new Promise((_, reject) => {
    timeoutId = setTimeout(
      () => reject(createPartnerFunctionTimeout()),
      PARTNER_CREATE_TIMEOUT_MS,
    );
  });

  try {
    return await Promise.race([
      supabase.functions.invoke(PARTNER_CREATE_FUNCTION_NAME, {
        body: payload,
        headers: {
...
async function createPartnerFromSecureServer(payload, session) {
  const { data, error } = await invokeSecurePartnerCreate(payload, session);

  if (error) {
    throw await normalizePartnerCreateFunctionError(error);
  }

  const partnerId =
    data?.partnerId ?? data?.parceiro_id ?? data?.profileId ?? data?.perfil_id ?? null;

  if (!partnerId) {
    throw new Error(
      "Cadastro criado sem identificador de parceiro retornado pelo servidor.",
    );
  }
  ...
  return {
    ...
    partnerTableMissing: Boolean(data?.partnerTableMissing),
  };
}
...
function mapScheduleFromRows(rows) {
  ...
  const findFirst = (candidates) =>
    rows.find((row) => candidates.includes(Number(row.dia_semana)));

  const weekRow = findFirst([1, 2, 3, 4, 5]);
  const saturdayRow = findFirst([6]);
  const sundayRow = findFirst([0]);
  ...
  const parseRow = (row, fallback) => {
    ...
    const openParts = scheduleTimeToParts(
      row.abre_as,
      fallback.openHour,
      fallback.openMinute,
    );
    const closeParts = scheduleTimeToParts(
      row.fecha_as,
      fallback.closeHour,
...
function mapCatalogProduct(product, institutionByProfileId) {
  return {
    ...
    name: product.titulo ?? "",
    institution:
      institutionByProfileId.get(product.perfil_parceiro_id) ??
...
function formatTime(hora, minuto) {
  return String(hora ?? 8).padStart(2, "0") + ":" + String(minuto ?? 0).padStart(2, "0") + ":00";
}
...
async function replacePartnerSchedule(partnerId, schedule) {
  ...
  const buildSafeScheduleRow = (diaIndex, key) => {
    const normalizedItem = normalized[key] ?? {};
    const sourceItem = source[key] ?? {};
    const fechado = !normalizedItem?.open;
    const hasOpenHour =
      sourceItem?.openHour !== undefined &&
      sourceItem?.openHour !== null &&
      String(sourceItem.openHour).trim() !== "";
    const hasOpenMinute =
      sourceItem?.openMinute !== undefined &&
      sourceItem?.openMinute !== null &&
      String(sourceItem.openMinute).trim() !== "";
    const hasCloseHour =
      sourceItem?.closeHour !== undefined &&
      sourceItem?.closeHour !== null &&
      String(sourceItem.closeHour).trim() !== "";
    const hasCloseMinute =
      sourceItem?.closeMinute !== undefined &&
      sourceItem?.closeMinute !== null &&
      String(sourceItem.closeMinute).trim() !== "";
    const horaAbertura = hasOpenHour ? normalizedItem.openHour : 8;
    const minutoAbertura = hasOpenMinute ? normalizedItem.openMinute : 0;
    const horaFechamento = hasCloseHour ? normalizedItem.closeHour : 18;
    const minutoFechamento = hasCloseMinute ? normalizedItem.closeMinute : 0;
    ...
    const row = fechado
      ...
      : {
          ...
          dia_semana: diaIndex,
          fechado: false,
          abre_as: formatTime(horaAbertura, minutoAbertura),
          fecha_as: formatTime(horaFechamento, minutoFechamento),
        };
  ...
  const rows = [
    buildSafeScheduleRow(1, "week"),
    buildSafeScheduleRow(2, "week"),
    buildSafeScheduleRow(3, "week"),
    buildSafeScheduleRow(4, "week"),
    buildSafeScheduleRow(5, "week"),
    buildSafeScheduleRow(6, "saturday"),
    buildSafeScheduleRow(0, "sunday"),
  ];
...
async function getRawPartnerProductById(profileId, productId) {
  const { data, error } = await euras
    .from("produtos")
    .select(
      "id, perfil_parceiro_id, titulo, descricao, preco_euras, url_imagem, ativo",
    )
    .eq("id", productId)
...
export function getPartnerApiErrorMessage(error) {
  ...
  if (!message) {
    return "NÃ£o foi possÃ­vel concluir a operaÃ§Ã£o no banco de parceiros.";
  }

  if (normalizedMessage.includes("tempo limite")) {
    return "ConexÃ£o com o banco demorou demais. Tente novamente em instantes.";
  }
  ...
  if (normalizedMessage.includes("infinite recursion detected in policy")) {
    return "Erro de permissÃ£o no banco (RLS). Rode o script de correÃ§Ã£o de policies e tente novamente.";
  }

  if (normalizedMessage.includes("row-level security")) {
    return "OperaÃ§Ã£o bloqueada por permissÃ£o (RLS). Verifique se seu usuÃ¡rio Ã© admin.";
  }

  if (
    normalizedMessage.includes("invalid input value for enum tipo_parceiro")
  ) {
    return "Valor de tipo de parceiro invÃ¡lido no schema atual. Atualize o schema ou rode o script de bootstrap final.";
  }
  ...
  if (
    ...
    return "Este e-mail ja esta cadastrado. Use outro e-mail para o parceiro.";
  }

  if (
    error?.code === "edge_function_fetch_error" ||
    error?.code === "edge_function_relay_error" ||
    normalizedMessage.includes("endpoint seguro de cadastro") ||
    normalizedMessage.includes("supabase functions")
  ) {
    return "Nao foi possivel acessar o endpoint seguro de cadastro. Verifique a conexao e tente novamente.";
  }
  ...
  if (
    ...
    normalizedMessage.includes("criar-parceiro") ||
    normalizedMessage.includes("admin-create-partner")
  ) {
    return "Funcao segura de cadastro nao encontrada. Faca o deploy da edge function criar-parceiro.";
  }

  if (error?.code === "23503") {
    return "NÃ£o foi possÃ­vel concluir a operaÃ§Ã£o por referÃªncia invÃ¡lida entre tabelas.";
  }

  if (error?.code === "23505") {
    return "JÃ¡ existe um registro com os mesmos dados Ãºnicos.";
  }

  if (normalizedMessage.includes("nao encontrado")) {
    return "Registro nÃ£o encontrado no banco.";
  }
...
export async function getPartnerById(partnerId) {
  ...
  if (partnerRow) {
    ...
    return {
      ...mappedPartner,
      schedule:
        scheduleMap.get(String(partnerRow.id)) ??
...
export async function createPartner({
  ...
  if (!normalizedUser) {
    throw new Error("Informe o nome do usuario responsavel.");
  }

  const normalizedPhone = phone?.trim() ?? "";
  if (!normalizedPhone) {
    throw new Error("Informe o numero do parceiro.");
  }
  ...
  const partnerCreation = await createPartnerFromSecureServer(
    {
      nome_instituicao: normalizedName,
      nome_usuario: normalizedUser,
  ...
  return partnerCreation.partnerId;
}

export async function updatePartner(
  partnerId,
  { name, user, phone, email, campus, imageUrl, schedule },
) {
  await ensureFreshSession();
  ...
  if (!partnerRow) {
    const profileUpdate = {
      ...
      ...(imageUrl?.trim() ? { url_avatar: imageUrl.trim() } : {}),
    };
  ...
  const normalizedName =
    name?.trim().toUpperCase() || partnerRow.nome_instituicao;
  const normalizedUser =
    user?.trim().toUpperCase() ??
    partnerRow.usuario_responsavel_nome ??
    normalizedName;
  const normalizedCampus =
    campus?.trim().toUpperCase() ?? partnerRow.campus ?? "";
  const normalizedImageUrl = imageUrl?.trim() || partnerRow.url_imagem || "";

  const { error: partnerError } = await updatePartnerWithCompatibleType(
    partnerId,
    {
      nome_instituicao: normalizedName,
      usuario_responsavel_nome: normalizedUser,
...
export async function listPartnerProducts(partnerId) {
  ...
  return runInFlightProductRequest(
    `partner-products:${partnerId}`,
    async () => {
      await ensureFreshSession();
      ...
      const payload = {
        ...
        products: (data ?? []).map((product) =>
          mapPartnerProduct(product, partner.name),
        ),
      };
...
export async function prefetchPartnerProducts(partnerId) {
  try {
    ...
    // Prefetch nÃ£o deve interromper a navegaÃ§Ã£o principal.
  }
}
...
export async function createPartnerProduct(
  ...
  if (Number.isNaN(numericPrice) || numericPrice < 0) {
    throw new Error("Informe um preco de Euras vÃ¡lido.");
  }

  const { error } = await euras.from("produtos").insert({
...
export async function updatePartnerProduct(
  ...
  const currentProduct = await getRawPartnerProductById(
    partner.profileId,
    productId,
  );
  if (!currentProduct) {
  ...
  const { error } = await euras
    ...
    .update({
      ...
      descricao: description?.trim() ?? "",
      preco_euras: numericPrice,
      ...(imageUrl?.trim() ? { url_imagem: imageUrl.trim() } : {}),
    })
    .eq("id", productId)
    .eq("perfil_parceiro_id", partner.profileId);
...
export async function listProducts() {
  ...
  return runInFlightProductRequest("catalog-products", async () => {
    ...
    const profileIds = [
      ...new Set(
        products.map((product) => product.perfil_parceiro_id).filter(Boolean),
      ),
    ];
    const institutionByProfileId =
...
export async function createProduct({
  ...
  const selectedProfileId = String(partnerProfileId ?? "").trim();
  let profileId = selectedProfileId || null;
  ...
  if (!profileId) {
    ...
    profileId = await resolvePartnerProfileIdByInstitution(
  ...
  if (!profileId) {
    throw new Error(
      "Instituição não cadastrada como parceiro. Cadastre o parceiro antes de criar o produto.",
    );
  }
...
export async function updateProduct(
  productId,
  { name, institution, description, priceEuras, imageUrl },
) {
  await ensureFreshSession();
  ...
  const normalizedInstitution = institution?.trim() ?? "";
  if (!normalizedInstitution) {
    throw new Error("Informe a instituiÃ§Ã£o do produto.");
  }

  const numericPrice = parseEurasValue(priceEuras);
  if (!Number.isFinite(numericPrice) || numericPrice < 0) {
    throw new Error("Informe um valor vÃ¡lido para o produto.");
  }

  const profileId = await resolvePartnerProfileIdByInstitution(
    normalizedInstitution,
  );
  if (!profileId) {
    throw new Error(
      "InstituiÃ§Ã£o nÃ£o cadastrada como parceiro. Cadastre o parceiro antes de salvar o produto.",
    );
  }
