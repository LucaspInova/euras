...
class ApiError extends Error {
  ...
  constructor(status: number, code: string, message: string, details?: string | null, hint?: string | null) {
    super(message)
    this.status = status
    this.code = code
...
function normalizeSchedule(input: unknown): WeeklySchedule {
  ...
  const normalizeItem = (key: 'week' | 'saturday' | 'sunday', fallbackOpen: boolean): DaySchedule => {
    const rawItem =
...
function isMissingRelationError(error: unknown) {
  ...
  return code === 'PGRST205' || code === '42P01' || message.includes('could not find the table') || message.includes('relation')
}
...
function mapUnknownError(error: unknown) {
  ...
  if (isEmailAlreadyExistsError(error)) {
    return new ApiError(409, 'user_already_exists', 'Este e-mail ja esta cadastrado para outro usuario.')
  }

  const code = String((error as { code?: string })?.code ?? '')
  const details = String((error as { details?: string })?.details ?? '')
  const hint = String((error as { hint?: string })?.hint ?? '')
  const message = String((error as { message?: string })?.message ?? 'Falha ao criar parceiro no servidor.')

  return new ApiError(400, code || 'partner_create_failed', message, details || null, hint || null)
}
...
async function ensureAdminCaller(userClient: any, userId: string) {
  ...
  if (!response.data?.id) {
    throw new ApiError(403, 'forbidden', 'Somente admins podem cadastrar parceiro.')
  }
}
...
async function upsertProfileWithCompatibility(
  ...
  if (authUserId) {
    ...
    const existingProfileId = (existingByAuthResponse.data?.id as string | undefined) ?? null
    ...
    if (existingProfileId) {
      ...
      if (updateResponse.error && isMissingColumnError(updateResponse.error, 'auth_user_id')) {
...
async function insertPartnerWithCompatibleType(
  ...
  const typeCandidates = buildPartnerTypeCandidates(String(payload.nome_instituicao ?? ''))
  ...
  if (fallback.error && lastEnumError && isInvalidPartnerTypeError(fallback.error)) {
...
async function upsertPartnerWithCompatibility(
  ...
  const updateAttempt = async (partnerId: string, payload: Record<string, unknown>) =>
    eurasAdmin
      .from('parceiros')
      .update(payload)
      .eq('id', partnerId)
  ...
  const existingPartnerId = (existingPartnerResponse.data?.id as string | undefined) ?? null

  if (existingPartnerId) {
    let updateResponse = await updateAttempt(existingPartnerId, payload)

    if (updateResponse.error && isMissingColumnError(updateResponse.error, 'perfil_id')) {
      payload = { ...payload }
  ...
  if (insertResponse.error && isMissingColumnError(insertResponse.error, 'perfil_id')) {
    const fallbackPayload = { ...payload }
    delete fallbackPayload.perfil_id
    insertResponse = await insertPartnerWithCompatibleType(eurasAdmin, fallbackPayload)
  }
  ...
  return {
    partnerId: (partnerAfterInsertResponse.data?.id as string | undefined) ?? null,
...
async function replacePartnerSchedule(
  ...
  const buildSafeRow = (day: number, item: DaySchedule) => {
...
async function rollbackPartnerCreation(
  ...
  if (authUserId) {
    const { error: removeAuthError } = await adminClient.auth.admin.deleteUser(authUserId)

    if (removeAuthError) {
      console.error('Falha ao reverter usuario Auth apos erro de cadastro.', removeAuthError)
    }
  }
}
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (authError || !authData.user?.id) {
      throw new ApiError(401, 'unauthorized', 'Sessao invalida. Faca login novamente.')
    }
    ...
    try {
      ...
      throw new ApiError(400, 'invalid_json', 'Payload invalido. Envie JSON valido.')
    }

    const nomeInstituicao = normalizeText(payload.nome_instituicao).toUpperCase()
    ...
    if (!nomeInstituicao) {
      throw new ApiError(400, 'validation_error', 'Informe o nome da instituicao.')
    }

    if (!nomeUsuario) {
      throw new ApiError(400, 'validation_error', 'Informe o nome do usuario responsavel.')
    }

    if (!numero) {
      throw new ApiError(400, 'validation_error', 'Informe o numero do parceiro.')
    }
    ...
    if (!senha || senha.length < 6) {
      throw new ApiError(400, 'validation_error', 'A senha deve ter no minimo 6 caracteres.')
    }

    const { data: perfilExistente, error: perfilExistenteError } = await eurasAdmin
      .from('perfis')
      .select('id')
      .eq('email', email)
      .limit(1)
    ...
    if (perfilExistenteError) {
      throw perfilExistenteError
    }

    if (perfilExistente?.id) {
      throw new ApiError(409, 'user_already_exists', 'Este e-mail ja esta cadastrado para outro usuario.')
    }
    ...
    if (!authUserId) {
      throw new ApiError(500, 'auth_user_id_missing', 'Nao foi possivel obter o id do usuario criado no Auth.')
    }

    createdAuthUserId = authUserId

    const { data: profileRow, error: profileError } = await upsertProfileWithCompatibility(eurasAdmin, {
      id: authUserId,
    ...
    const partnerUpsert = await upsertPartnerWithCompatibility(eurasAdmin, partnerPayload)
    ...
    if (createdAuthUserId) {
      await rollbackPartnerCreation(eurasAdmin, adminClient, createdProfileId, createdPartnerId, createdAuthUserId)
    }
